...
class JWTHandler:
    ...
    @staticmethod
    def create_jwt_token(user_id: UUID, token_type: Literal['access', 'refresh']) -> tuple[str, datetime]:
        """Create jwt token"""
        ...
        token = jwt.encode(jwt_data, settings.JWT_SECRET_KEY.get_secret_value(), algorithm=settings.JWT_ALGORITHM)
