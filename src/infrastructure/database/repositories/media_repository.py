...
class MediaRepository(SQLAlchemyRepository[Media]):
    ...
    async def get_conversation_media(self, conversation_id: UUID, include_message: bool = True) -> list[Media]:
    ...
    async def get_media_by_id(self, media_id: UUID, include_message: bool = True) -> Media | None:
    ...
    async def get_media_by_ids(self, media_ids: list[UUID], include_message: bool = True) -> list[Media]:
