...
class MessageRepository(SQLAlchemyRepository[Messages]):
    ...
    async def get_conversation_messages(
        ...
        if user_id:
            participant_query = select(UserConversation).where(
                and_(UserConversation.user_id == user_id, UserConversation.conversation_id == conversation_id)
            )
            ...
            if participant and participant.last_read_message_id:
                ...
                last_read_query = select(self.model).where(self.model.id == participant.last_read_message_id)
        ...
        if last_read_message:
            ...
            unread_query = select(self.model).where(
                and_(*base_conditions, self.model.created_at > last_read_message.created_at)
            )
            unread_query = unread_query.options(
                selectinload(self.model.sender).selectinload(Users.avatar), selectinload(self.model.media)
            )
    ...
    async def get_message_readers(
        ...
        messages_query = select(self.model).where(
            and_(self.model.id.in_(message_ids), self.model.conversation_id == conversation_id)
        )
        ...
        participants_query = (
            ...
            .join(Users, UserConversation.user_id == Users.id)
            .where(
                UserConversation.conversation_id == conversation_id, UserConversation.last_read_message_id.isnot(None)
            )
        )
        ...
        last_read_message_ids = {p.last_read_message_id for p in participants if p.last_read_message_id}

        if not last_read_message_ids:
            return {msg_id: [] for msg_id in message_ids}

        last_read_messages_query = select(self.model).where(self.model.id.in_(last_read_message_ids))
        last_read_messages_result = await self._session.execute(last_read_messages_query)
        last_read_messages = {msg.id: msg for msg in last_read_messages_result.scalars().all()}
    ...
    async def search_messages(
        ...
        query = (
            ...
            .options(selectinload(self.model.sender).selectinload(Users.avatar), selectinload(self.model.media))
            .order_by(self.model.created_at.desc())
            .offset(skip)
            .limit(limit)
        )
