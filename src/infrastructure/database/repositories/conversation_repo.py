...
class ConversationRepository(SQLAlchemyRepository[Conversations]):
    ...
    async def remove_participant(self, user_id: UUID, conversation_id: UUID) -> bool:
    ...
    async def get_participants(self, conversation_id: UUID, include_user: bool = True) -> list[UserConversation]:
        """Get all participants of a conversation"""
        query = select(UserConversation).where(UserConversation.conversation_id == conversation_id)

        if include_user:
            query = query.options(selectinload(UserConversation.user).selectinload(Users.avatar))
    ...
    async def get_user_role(self, user_id: UUID, conversation_id: UUID) -> Roles | None:
    ...
    async def get_user_conversations(
        ...
        last_message_subquery = (
            select(func.max(Messages.created_at).label('last_message_at'))
    ...
    async def update_last_read_message_id(
        ...
        message_query = select(Messages).where(
            and_(Messages.id == message_id, Messages.conversation_id == conversation_id, Messages.is_deleted == False)
        )
    ...
    async def _get_participant(self, user_id: UUID, conversation_id: UUID) -> UserConversation | None:
