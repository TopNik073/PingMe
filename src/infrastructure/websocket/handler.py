...
class WebSocketHandler:
    ...
    async def authenticate(self, token: str) -> bool:
        ...
        try:
            ...
            # Update user status
            await self.user_repository.update_online_status_and_last_seen(user_id, is_online=True)
    ...
    async def handle_auth(self, message: WSAuthMessage):
        ...
        if self.authenticated:
            await self.send_error(WSErrorCode.INVALID_MESSAGE, 'Already authenticated')
            return

        success = await self.authenticate(message.token)
        if success:
            auth_success = WSAuthSuccess(user_id=self.user_id, user_name=self.user_name)
            await self._send_message_with_sequence(auth_success)
        else:
            await self.send_error(WSErrorCode.AUTH_FAILED, 'Authentication failed. Invalid or expired token.')
    ...
    async def handle_message_create(self, message: WSMessageCreate):
        ...
        try:
            ...
            await self.connection_manager.subscribe_to_conversation(self.user_id, message.conversation_id)

            # Get participants
            participant_ids = await self.message_service.get_conversation_participants(message.conversation_id)

            # Prepare media list for response
            media_list = []
            if created_message.media:
                media_list = [MediaResponse.model_validate(media) for media in created_message.media]
            ...
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Failed to create message')
    ...
    async def handle_message_edit(self, message: WSMessageEdit):
        ...
        try:
            ...
            # Get participants
            participant_ids = await self.message_service.get_conversation_participants(edited_message.conversation_id)
            ...
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Failed to edit message')
    ...
    async def handle_message_delete(self, message: WSMessageDelete):
        ...
        try:
            ...
            # Get participants
            participant_ids = await self.message_service.get_conversation_participants(deleted_message.conversation_id)
            ...
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Failed to delete message')
    ...
    async def handle_message_forward(self, message: WSMessageForward):
        ...
        try:
            ...
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Failed to forward message')
    ...
    async def handle_typing_start(self, message: WSTypingStart):
        ...
        try:
            ...
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Failed to handle typing indicator')
    ...
    async def handle_typing_stop_internal(self, conversation_id: UUID):
        ...
        try:
            # Get participants
            participant_ids = await self.message_service.get_conversation_participants(conversation_id)
    ...
    async def _heartbeat_monitor(self):
        ...
        while True:
            ...
            if time_since_ping > settings.WS_HEARTBEAT_TIMEOUT:
                logger.warning(
                    'Heartbeat timeout for user %s (%.1f seconds since last ping)', self.user_id, time_since_ping
                )
        ...
        try:
            ...
            if success:
                ...
                await self.send_error(
                    WSErrorCode.MESSAGE_NOT_FOUND, "Message not found or you don't have permission to mark it as read"
                )
        except Exception as e:
            logger.exception('Failed to mark message as read: %s', e)
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Failed to mark message as read')
    ...
    async def handle_ack(self, message: WSAck):
        ...
        logger.debug(
            'Message %s acknowledged by user %s (sequence: %s)', message.message_id, self.user_id, message.sequence
        )
    ...
    async def handle_subscribe(self, message: WSSubscribe):
        ...
        try:
            ...
            if self.user_id not in participant_ids:
                await self.send_error(WSErrorCode.PERMISSION_DENIED, 'You are not a participant of this conversation')
                return

            # Subscribe to conversation
            await self.connection_manager.subscribe_to_conversation(self.user_id, message.conversation_id)

            logger.debug('User %s subscribed to conversation %s', self.user_id, message.conversation_id)
        except Exception as e:
            logger.exception('Failed to subscribe to conversation: %s', e)
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Failed to subscribe to conversation')
    ...
    async def handle_unsubscribe(self, message: WSUnsubscribe):
        ...
        try:
            ...
            await self.connection_manager.unsubscribe_from_conversation(self.user_id, message.conversation_id)

            logger.debug('User %s unsubscribed from conversation %s', self.user_id, message.conversation_id)
        except Exception as e:
            logger.exception('Failed to unsubscribe from conversation: %s', e)
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Failed to unsubscribe from conversation')
    ...
    async def process_message(self, raw_message: str):
        ...
        try:
            ...
            if not message_type:
                await self.send_error(WSErrorCode.INVALID_MESSAGE, 'Message type is required')
                return

            # Check rate limit (skip for pong responses)
            if message_type != 'pong' and not rate_limiter.is_allowed(self.user_id, message_type):
                await self.send_error(
                    WSErrorCode.RATE_LIMIT_EXCEEDED, f"Rate limit exceeded for message type '{message_type}'"
                )
                return
            ...
            if handler_info:
                ...
                try:
                    ...
                    await self.send_error(
                        WSErrorCode.INVALID_MESSAGE, f"Invalid message format for type '{message_type}': {e!s}"
                    )
                except Exception as e:
                    logger.exception('Failed to process %s message: %s', message_type, e)
                    await self.send_error(
                        WSErrorCode.INTERNAL_ERROR, f'Internal error processing {message_type} message'
                    )
            else:
                await self.send_error(WSErrorCode.INVALID_MESSAGE, f'Unknown message type: {message_type}')
        except json.JSONDecodeError:
            await self.send_error(WSErrorCode.INVALID_MESSAGE, 'Invalid JSON format')
        except Exception as e:
            logger.exception('Error processing message: %s', e)
            await self.send_error(WSErrorCode.INTERNAL_ERROR, 'Internal server error')
        ...
        if self.user_id:
            ...
            if disconnected_user_id:
                ...
                if not self.connection_manager.is_user_online(disconnected_user_id):
                    await self.user_repository.update_online_status_and_last_seen(disconnected_user_id, is_online=False)
                    await self.session.commit()
