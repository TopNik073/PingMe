...
class ConnectionManager:
    ...
    async def disconnect(self, websocket: WebSocket) -> UUID | None:
        ...
        async with self._lock:
            ...
            if user_id and user_id in self.active_connections:
                ...
                logger.info("User %s disconnected. Remaining connections: %s", user_id, len(self.active_connections.get(user_id, set())))
            ...
            if self.is_user_online(user_id):
                ...
                for connection in connections:
                    try:
                        ...
                        logger.warning("Failed to send message to user %s in conversation %s: %s", user_id, conversation_id, e)
                        await self.disconnect(connection)
    ...
    async def broadcast_to_conversation_participants(
        ...
        exclude_user_id: list[UUID] | None = None
    ) -> list[UUID]:
        """
