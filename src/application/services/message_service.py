...
class MessageService:
    ...
    async def create_message(  # noqa: PLR0912
        ...
        try:
            ...
            if forwarded_from_id:
                original_message = await self._message_repo.get_message_by_id(forwarded_from_id, include_deleted=False)
    ...
    async def edit_message(
        ...
        try:
            ...
            is_participant = await self._conversation_repo.is_participant(user_id, message.conversation_id)
            if not is_participant:
                raise ValueError('User is not a participant of this conversation')
    ...
    async def forward_message(
        ...
        try:
            ...
            original_message = await self._message_repo.get_message_by_id(message_id, include_deleted=False)
            if not original_message:
                raise ValueError('Original message not found or deleted')
            ...
            is_participant_target = await self._conversation_repo.is_participant(user_id, target_conversation_id)
    ...
    async def _notify_offline_users(self, message: Messages):
        ...
        try:
            ...
            offline_users = [
                ...
                if uid != message.sender_id and not self._connection_manager.is_user_online(uid)
            ]
