...
class MediaService:
    ...
    async def get_media_by_id(self, media_id: UUID, user_id: UUID) -> Media:
    ...
    async def get_conversation_media(self, conversation_id: UUID, user_id: UUID) -> list[Media]:
        ...
        try:
            ...
            # Get media
            return await self._media_repo.get_conversation_media(conversation_id, include_message=True)
        except Exception as e:
    ...
    async def get_media_file(self, media_id: UUID, user_id: UUID) -> tuple[bytes, str, str]:
        """
