...
def raise_http_exception(
    ...
    raise HTTPException(
        status_code=status_code, detail=response_error(error=error_str, message=message)
    )


def raise_validation_error(message: str, error: Exception) -> None:
    raise_http_exception(
        status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, message=message, error=error
    )
