...
class WebSocketTestClient:
    ...
    async def test_message_delete(self, message_id: UUID) -> bool:
        ...
        success = (
            response and response.get('type') == 'message_delete' and response.get('message_id') == str(message_id)
        )
...
class MultiClientTestRunner:
    ...
    async def test_broadcast_message_create(
        self, client_a: WebSocketTestClient, client_b: WebSocketTestClient, conversation_id: UUID, content: str
    ):
        ...
        listen_task = asyncio.create_task(client_b.wait_for_message_type('message', timeout=10.0))
        ...
        success = (
            ...
            and received.get('type') == 'message'
            and UUID(received.get('id')) == message_id
        )
        self._record_test_result(success, 'Broadcast: Message Create')
        return message_id, success

    async def test_broadcast_message_edit(
        self, client_a: WebSocketTestClient, client_b: WebSocketTestClient, message_id: UUID, new_content: str
    ):
        ...
        listen_task = asyncio.create_task(client_b.wait_for_message_type('message_edit', timeout=10.0))
        ...
        success = (
            ...
            and received.get('type') == 'message_edit'
            and UUID(received.get('message_id')) == message_id
            and received.get('content') == new_content
        )
        self._record_test_result(success, 'Broadcast: Message Edit')
        return success

    async def test_broadcast_message_delete(
        self, client_a: WebSocketTestClient, client_b: WebSocketTestClient, message_id: UUID
    ):
        ...
        listen_task = asyncio.create_task(client_b.wait_for_message_type('message_delete', timeout=10.0))
        ...
        success = (
            received is not None
            and received.get('type') == 'message_delete'
            and UUID(received.get('message_id')) == message_id
        )
        self._record_test_result(success, 'Broadcast: Message Delete')
        return success

    async def test_broadcast_message_forward(
        self,
        client_a: WebSocketTestClient,
        client_b: WebSocketTestClient,
        message_id: UUID,
        ...
        listen_task = asyncio.create_task(client_b.wait_for_message_type('message_forward', timeout=10.0))
        ...
        success = (
            new_message_id is not None
            and received is not None
            and received.get('type') == 'message_forward'
            and UUID(received.get('new_message_id')) == new_message_id
        )
        self._record_test_result(success, 'Broadcast: Message Forward')
        return success

    async def test_broadcast_typing_indicator(
        self, client_a: WebSocketTestClient, client_b: WebSocketTestClient, conversation_id: UUID
    ):
        ...
        listen_start_task = asyncio.create_task(client_b.wait_for_message_type('typing_start', timeout=5.0))
        ...
        success_start = (
            received_start is not None
            and received_start.get('type') == 'typing_start'
            and UUID(received_start.get('user_id')) == client_a.user_id
        )

        # Client B listens for typing_stop
        listen_stop_task = asyncio.create_task(client_b.wait_for_message_type('typing_stop', timeout=5.0))
        ...
        success_stop = (
            received_stop is not None
            and received_stop.get('type') == 'typing_stop'
            and UUID(received_stop.get('user_id')) == client_a.user_id
        )
    ...
    async def test_broadcast_message_read(
        self, client_a: WebSocketTestClient, client_b: WebSocketTestClient, message_id: UUID, conversation_id: UUID
    ):
        ...
        listen_task = asyncio.create_task(client_b.wait_for_message_type('message_read', timeout=10.0))
        ...
        success = (
            received is not None
            and received.get('type') == 'message_read'
            and UUID(received.get('message_id')) == message_id
            and UUID(received.get('reader_id')) == client_a.user_id
        )
    ...
    async def test_user_online_offline(self, client_a: WebSocketTestClient, client_b: WebSocketTestClient):
        ...
        listen_offline_task = asyncio.create_task(client_b.wait_for_message_type('user_offline', timeout=5.0))
...
async def run_single_client_tests(args):
    ...
    message_id = None
    if args.conversation_id:
        message_id = await client.test_send_message(UUID(args.conversation_id), args.test_message)

        # Test editing message (if we got a message_id)
        if message_id:
            await client.test_message_edit(message_id, f'{args.test_message} (edited)')
        ...
        if message_id and args.target_conversation_id:
            ...
            new_message_id = await client.test_send_message(UUID(args.conversation_id), 'Message to forward')
            if new_message_id:
                await client.test_message_forward(new_message_id, UUID(args.target_conversation_id))

        # Test mark read (need a message that exists and wasn't deleted)
        # Create a new message for mark_read test
        mark_read_message_id = await client.test_send_message(UUID(args.conversation_id), 'Message for mark read test')
...
async def run_multi_client_tests(args):
    ...
    try:
        ...
        await runner.test_broadcast_typing_indicator(client_a, client_b, conversation_id)

        # Test broadcast: message read (need a new message that wasn't deleted)
        read_test_message_id = await client_a.test_send_message(conversation_id, 'Message for read test')
        if read_test_message_id:
            await runner.test_broadcast_message_read(client_a, client_b, read_test_message_id, conversation_id)
        ...
        if args.target_conversation_id:
            ...
            forward_message_id = await client_a.test_send_message(conversation_id, 'Message to forward')
